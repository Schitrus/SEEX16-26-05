...
def parseFits(fits_dir, user_files = None, banlist=False):
    ...
    if user_files:
        for user_file in user_files:
            user_datas.append(np.loadtxt(fname='user_data/' + user_file, skiprows= 3, unpack=True, encoding='latin1', 
                                                    dtype=[("time", '<U20'), ("ra_pixels",float), ("dec_pixels", float)]))
            user_names.append(user_datas[-1][0])
            user_ra_pixelss.append(user_datas[-1][1])
            user_dec_pixelss.append(user_datas[-1][2]) 
    ...
    for fits_file in fits_files:
        ...
        # Annars fortsätt som vanligt
            ...
            if user_files:
                user_radecs = []
                for user_name, user_ra_pixels, user_dec_pixels in zip(user_names, user_ra_pixelss, user_dec_pixelss):
                    ...
                    user_radecs.append((np.abs(ra_delta), np.abs(dec_delta), user_ra, user_dec))
